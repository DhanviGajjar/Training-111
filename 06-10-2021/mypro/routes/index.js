...
router.get('/pro-addtocart/:id', function(req, res, next) {
  var addtocartid = req.params.id;
  ProModel.findById(addtocartid,function(err,data){
    if(err){
      console.log("Error in Edit" + err)
    }else{
      console.log(data);
      res.render('product/pro-addtocart',{mydata:data})
    }
  }).lean();
...
router.post('/pro-addtocart/:id', function(req, res, next) {
  var addtocartid = req.params.id;
  ...
  const mybodydata = {
    ...
    _product:addtocartid
  }
  console.log('cartdata',mybodydata);
 ...
 CartModel.find(function(err,data){
      ...
      .lean()
        .exec(function(err, data) {
          ...
          res.render("product/display-cart", { mydata: data});
        })
      });
});
router.get('/cart-delete/:id', function(req, res, next) {
  var deletecid = req.params.id;
CartModel.findByIdAndDelete(deletecid,function(err,data){
    if(err)
    {
      console.log("Error in Delete " + err);
    }else{
      console.log("Record Deleted " + deletecid);
...
router.get('/cart-edit/:id', function(req, res, next) {
  var editcid = req.params.id;
  CartModel.findById(editcid,function(err,data){
    if(err){
      console.log("Error in Edit" + err)
    }else{
      console.log(data);
      res.render('product/cart-edit',{mydata:data})
    }
  }).lean();

});
router.post('/cart-edit/:id', function(req, res, next) {
  var editcid = req.params.id;
  const mybodydata = {
    pro_name : req.body.pname,
    pro_price:req.body.pprice, 
  ...
  CartModel.findByIdAndUpdate(editcid,mybodydata,function(err,data){
    if(err){
      console.log("Error in Edit" + err)
    }else{
      console.log( "Record Updated" +  data);
